...
PIMTraceBasedCPU::PIMTraceBasedCPU(const std::string& config_file,
                             const std::string& output_dir,
                             const std::string& trace_file)
    : CPU(config_file, output_dir) {
    trace_file_.open(trace_file);
...
void PIMTraceBasedCPU::ClockTick() {
    // Advance the memory system clock (maintaining local tick correctness)
    memory_system_.ClockTick();

    if (!trace_file_.eof()) {
        if (get_next_) {
            get_next_ = false;
            trace_file_ >> cmd_str >> trans_.added_cycle;
        }

        if (trans_.added_cycle <= clk_) {

            uint64_t target_addr = 0x0;
            bool is_write = false; // Assuming command submissions are writes

            if (cmd_str == "PIM_START") {
                target_addr = PIM_START_ADDR;
            } else if (cmd_str == "MEM_PAUSE") {
                target_addr = PIM_RESUME_ADDR;
            } else if (cmd_str == "MEM_RESUME") {
                target_addr = PIM_PAUSE_ADDR;
            } else {
                target_addr = trans_.addr;
                is_write = trans_.is_write;
            }

            get_next_ = memory_system_.WillAcceptTransaction(target_addr, is_write);

            if (get_next_) {
                memory_system_.AddTransaction(target_addr, is_write);
            }
        }
    } else {
        memory_system_.AddTransaction(PIM_QUERY_ADDR, false);
